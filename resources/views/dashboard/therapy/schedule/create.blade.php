                ...
                <div class="text-gray-900">
                    <div class="flex min-w-full min-h-screen">
                        ...
                        <div class="flex flex-col flex-grow min-h-screen gap-4 p-6">
                            ...
                            <form class="flex flex-col items-start justify-start flex-grow gap-2" action="{{ route('schedule.store') }}" method="POST">
                                @csrf
                                <div class="flex flex-col gap-1">
                                    <label for="start_time">Start Time (HH:MM format, 24-hour clock)</label>
                                    <input type="text" name="start_time" class="rounded-md form-input bg-healthymind-bg" placeholder="08:00">
                                </div>
                                <div class="flex flex-col gap-1">
                                    <label for="end_time">End Time (HH:MM format, 24-hour clock)</label>
                                    <input type="text" name="end_time" class="rounded-md form-input bg-healthymind-bg" placeholder="10:00">
                                </div>
